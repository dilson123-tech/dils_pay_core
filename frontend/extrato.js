...
function showBanner(msg, type="ok"){
  ...
  clearTimeout(showBanner._t); showBanner._t = setTimeout(()=> el.style.display="none", 3500);
}
...
/* Datas helpers */
function ensureIsoStart(v){ return v ? (v.length === 10 ? `${v}T00:00:00` : v) : ""; }
function ensureIsoEnd(v){   return v ? (v.length === 10 ? `${v}T23:59:59` : v) : ""; }
...
function renderSkeletonRows(n = state.pageSize || 10) {
 ...
 function normalizeRow(it){
  ...
  let valor =
    it.valor ?? it.amount ?? it.value ?? it.total ?? (('valor_centavos' in it) ? (Number(it.valor_centavos)||0)/100 : 0);
...
function readFiltersFromForm(){
  ...
  state.tipo     = (($("tipo")?.value || "").toUpperCase() === "TODOS") ? "" : (($("tipo")?.value || "").toUpperCase());
...
function decodeJwtPayload(t){
  try{
    const b = (t || '').split('.')[1]; if (!b) return null;
...
function updateTokenTTL(){
  ...
  if (!tok){ el.textContent = ''; el.style.color=''; return; }

  const p = decodeJwtPayload(tok);
  if (!p?.exp){ el.textContent = ''; el.style.color=''; return; }
  ...
  _autoRefTimer = setTimeout(async ()=>{
    try{
      ...
      if(access_token){
        ...
        const tf = $('token'); if(tf) tf.value = access_token;
        updateTokenTTL();
...
(function loginBox(){
  ...
  bRefresh?.addEventListener('click', async (e) => {
    ...
    try {
      const base = window.apiBase?.(); if (!base) throw new Error("BASE_URL vazio");
      const rt = localStorage.getItem('REFRESH_TOKEN'); if (!rt) throw new Error("Sem refresh_token salvo");
  ...
  async function doLogin(){
    try{
      ...
      refreshAuthButtons();
      p && (p.value = "");
    }catch(e){ SB(`Falha no login: ${e.message||e}`, "error"); }
  }
  ...
  async function doLoginDev(){
    try{
      ...
      refreshAuthButtons();
    }catch(e){ SB(`Falha no login_dev: ${e.message||e}`, "error"); }
  }
  ...
  bLogin  && bLogin.addEventListener('click', (e)=>{ e.preventDefault(); doLogin(); });
  bDev    && bDev.addEventListener('click',   (e)=>{ e.preventDefault(); doLoginDev(); });
  bLogout && bLogout.addEventListener('click', doLogout);
  [u,p].forEach(el => el && el.addEventListener('keydown', ev=>{
...
function wireUI(){
  ...
  // Filtros
  $("aplicar")?.addEventListener("click", ()=>{ state.page = 1; saveFiltersToLS(); fetchAndRender(); });
  $("limpar") ?.addEventListener("click", ()=>{
    if ($("dataIni")) $("dataIni").value = "";
    if ($("dataFim")) $("dataFim").value = "";
    $("tipo").value = ""; $("pageSize").value = String(10);
    localStorage.setItem("FILTROS", JSON.stringify({dataIni:"",dataFim:"",tipo:"",pageSize:"10"}));
  ...
  $("presetHoje")?.addEventListener("click", ()=>{
    ...
    $("dataIni").value = d; $("dataFim").value = d; state.page = 1; saveFiltersToLS(); fetchAndRender();
  });
  $("preset7")?.addEventListener("click", ()=>{
    const e=new Date(); const s=new Date(); s.setDate(e.getDate()-6);
    $("dataIni").value=s.toISOString().slice(0,10); $("dataFim").value=e.toISOString().slice(0,10); state.page=1; saveFiltersToLS(); fetchAndRender();
  });
  ...
  // Page size
  $("pageSize")?.addEventListener("change", ()=>{ state.page=1; saveFiltersToLS(); fetchAndRender(); });
  ...
  $("btnCsvPagina") ?.addEventListener("click", (e)=>{ e.preventDefault(); openCsv('page'); });
  $("btnCsvTudo")   ?.addEventListener("click", (e)=>{ e.preventDefault(); openCsv('all'); });

  // Testar conexão
  $("testConn")?.addEventListener("click", (e)=>{ e.preventDefault(); testConn(); });
}
...
function killBackdrops(){
  ...
  document.querySelectorAll('#overlay,#loading,.modal-backdrop,.blocker,[class*=overlay],[class*=backdrop]')
    .forEach(el => { el.style.pointerEvents='none'; el.style.opacity='0'; el.style.zIndex='-1'; });
}
...
async function boot(){
  ...
  killBackdrops(); setTimeout(killBackdrops, 120);
}
